...
def calculateAvgRating(bnss_statistics, ratings, timeKey, total_time_slots):
    if StatConstants.AVERAGE_RATING not in bnss_statistics:
        bnss_statistics[StatConstants.AVERAGE_RATING] = numpy.zeros(total_time_slots)
    bnss_statistics[StatConstants.AVERAGE_RATING][timeKey] = float(sum(ratings))


def calculateRatingEntropy(bnss_statistics, ratings, reviews_for_bnss, timeKey, total_time_slots):
    sorted_rating_list = set(sorted(ratings))

    if StatConstants.RATING_DISTRIBUTION not in bnss_statistics:
        bnss_statistics[StatConstants.RATING_DISTRIBUTION] = dict()

    if StatConstants.RATING_ENTROPY not in bnss_statistics:
        bnss_statistics[StatConstants.RATING_ENTROPY] = numpy.zeros(total_time_slots)

    if timeKey not in bnss_statistics[StatConstants.RATING_DISTRIBUTION]:
        bnss_statistics[StatConstants.RATING_DISTRIBUTION][timeKey] = {key: 0.0 for key in sorted_rating_list}

    for rating in ratings:
        bnss_statistics[StatConstants.RATING_DISTRIBUTION][timeKey][rating] += 1.0

    for rating in sorted_rating_list:
        bnss_statistics[StatConstants.RATING_DISTRIBUTION][timeKey][rating] /= float(len(reviews_for_bnss))

    if timeKey in bnss_statistics[StatConstants.RATING_DISTRIBUTION]:
            entropy = entropyFn(bnss_statistics[StatConstants.RATING_DISTRIBUTION][timeKey])
            bnss_statistics[StatConstants.RATING_ENTROPY][timeKey] = entropy

def calculateNoOfReviews(bnss_statistics, neighboring_usr_nodes, timeKey, total_time_slots):
    if StatConstants.NO_OF_REVIEWS not in bnss_statistics:
        bnss_statistics[StatConstants.NO_OF_REVIEWS] = numpy.zeros(total_time_slots, dtype=int)
    noOfReviews = len(neighboring_usr_nodes)
    bnss_statistics[StatConstants.NO_OF_REVIEWS][timeKey] = noOfReviews
    return noOfReviews


def calculateRatioOfSingletons(bnss_statistics, neighboring_usr_nodes, reviews_for_bnss, superGraph, timeKey,
                               total_time_slots):
    if StatConstants.RATIO_OF_SINGLETONS not in bnss_statistics:
        bnss_statistics[StatConstants.RATIO_OF_SINGLETONS] = numpy.zeros(total_time_slots)
    noOfSingleTons = 0
    for neighbor in neighboring_usr_nodes:
        if len(superGraph.neighbors(neighbor)) == 1:
            noOfSingleTons += 1
    bnss_statistics[StatConstants.RATIO_OF_SINGLETONS][timeKey] = float(noOfSingleTons) / float(
        len(reviews_for_bnss))


def calculateRatioOfFirstTimers(G, bnss_statistics, neighboring_usr_nodes, reviews_for_bnss, superGraph,
                                timeKey, total_time_slots):
    if StatConstants.RATIO_OF_FIRST_TIMERS not in bnss_statistics:
        bnss_statistics[StatConstants.RATIO_OF_FIRST_TIMERS] = numpy.zeros(total_time_slots)
    noOfFirstTimers = 0
    for usr_neighbor in neighboring_usr_nodes:
        ...
        if firstReview.getId() == current_temporal_review.getId():
            noOfFirstTimers += 1
    bnss_statistics[StatConstants.RATIO_OF_FIRST_TIMERS][timeKey] = float(noOfFirstTimers) / float(
        len(reviews_for_bnss))


def calculateYouthScore(G, bnss_statistics, neighboring_usr_nodes, superGraph, timeKey, total_time_slots):
    if StatConstants.YOUTH_SCORE not in bnss_statistics:
        bnss_statistics[StatConstants.YOUTH_SCORE] = numpy.zeros(total_time_slots)
    youth_scores = []
    for usr_neighbor in neighboring_usr_nodes:
        (usrId, usr_type) = usr_neighbor
        allReviews = [superGraph.getReview(usrId, super_graph_bnssId) \
    ...
    bnss_statistics[StatConstants.YOUTH_SCORE][timeKey] = numpy.mean(numpy.array(youth_scores))


def calculateTemporalEntropyScore(G, bnss_statistics, neighboring_usr_nodes, noOfReviews, timeKey, timeLength,
                                  total_time_slots):
    entropyScore = 0
    if StatConstants.ENTROPY_SCORE not in bnss_statistics:
        bnss_statistics[StatConstants.ENTROPY_SCORE] = numpy.zeros(total_time_slots)
    if noOfReviews >= 2:
        ...
        if StatConstants.REVIEW_TIME_VELOCITY not in bnss_statistics:
            bnss_statistics[StatConstants.REVIEW_TIME_VELOCITY] = dict()

        bnss_statistics[StatConstants.REVIEW_TIME_VELOCITY][timeKey] = allReviewVelocity
        ...
        bnss_statistics[StatConstants.ENTROPY_SCORE][timeKey] = entropyScore

def calculateMaxTextSimilarity(G, bnss_statistics, neighboring_usr_nodes, noOfReviews, timeKey, timeLength,
                                  total_time_slots):
    if StatConstants.MAX_TEXT_SIMILARITY not in bnss_statistics:
        bnss_statistics[StatConstants.MAX_TEXT_SIMILARITY] = numpy.zeros(total_time_slots)
    ...
    bnss_statistics[StatConstants.MAX_TEXT_SIMILARITY][timeKey] = maxTextSimilarity
