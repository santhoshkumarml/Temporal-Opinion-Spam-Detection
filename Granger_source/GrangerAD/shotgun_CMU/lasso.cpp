...
void main_optimization_loop(double lambda, int regpathlength, double threshold, int maxiter, int verbose) {
    ...
    do {
        ...
        if (iterations >= maxiter && maxiter > 0) {
            if(verbose) mexPrintf("Exceeded max iterations: %d\n", maxiter);
            return;
        }
        double maxChange=0;
        ...
        if (verbose){
          ...
          long time = (long)floor(1000*(omp_get_wtime() - start_time));
          mexPrintf("Iteration: %d Clock: %lu Objective: %g L1 : %g L2err: %g L0 %d\n", 
                  iterations, time, obj, l1x, l2err, (int)l0x);
        }
    } while (regularization_path_step >= 0);
    delete[] delta;
    if(verbose) mexPrintf("Num of shoots = %lld\n", num_of_shoots);
}
