...
{     
    ...
    if (numthreads>0) {
        ...
        if(verbose) mexPrintf("OMP threads = %d\n", numthreads);
        #endif
    }
    ...
    d = mxGetN(prhs[0]);
    n = mxGetM(prhs[0]);
    if(verbose) mexPrintf("d = %d, n=%d, y-dim=%d\n", d,n, mxGetM(prhs[1]));
    ...
    if (mxIsSparse(prhs[0])) {
        // sparse
        if(verbose) mexPrintf("A was sparse\n");
		...
		if (jccur==0) continue;
         ...
         if(verbose) mexPrintf("Loaded A\n");
         
        
   } else {
       if(verbose) mexPrintf("A was dense.\n");
       double * Ad = mxGetPr(A);
       int i = 0;
